...
ggplot(data = diamonds) +
  geom_point(mapping = aes(x = carat, y = price, color = "blue", shape = cut))
...
ggplot(data = diamonds.sample) +
  geom_point(mapping = aes(x = carat, y = cut, size=price, color = carat))

# That's kind of messy. Try using `smooth` geometry instead.
ggplot(data = diamonds.sample) +
  geom_smooth(mapping = aes(x = carat, y = cut, size=price, color = carat))
...
ggplot(data = diamonds.sample) +
  geom_histogram(mapping = aes(x = carat, y = cut, fill = clarity), alpha = 0.1)
